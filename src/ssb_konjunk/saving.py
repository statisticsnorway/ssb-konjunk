...
def get_saved_file(name:str, datatilstand:str, bucket_path:str, year:Union[int, str] = '', month:Union[int, str] = '', day:Union[int, str] = '', filetype:str = 'parquet', fs:dapla.gcs.GCSFileSystem = None , seperator:str = ';', end_year:Union[int, str] = '', end_month:Union[int, str] = '', end_day:Union[int, str] = '') -> pd.DataFrame: 
    """
    ...
    filename = get_time_period_standard(base_name = name, 
                                        start_year = year, 
                                        start_month = month, 
                                        end_year = end_year, 
                                        end_month = end_month)
    ...
    versions = get_versions(bucket_path + datatilstand+'/', filename, re.compile(rf'{filename}(\d+)\.{filetype}'))
    
    if len(versions) == 0:
        print(f"There are no such files. {filename}")
        return pd.DataFrame()
    else:
        path = bucket_path + datatilstand + '/' + filename + f'{versions[-1]}.{filetype}'
...
def get_time_period_standard(base_name:str, start_year:Union[int, str] = '', start_month:Union[int, str] = '', start_day:Union[int, str] = '', end_year:Union[int, str] = '', end_month:Union[int, str] = '', end_day:Union[int, str] = '') -> str:
    """
...
def get_versions(folder_path:str, filename:str, filename_pattern: Pattern[str]) -> list[str]:
    """
