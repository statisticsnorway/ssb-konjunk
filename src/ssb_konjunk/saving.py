...
def get_ssb_file(
    ...
    file_path = structure_ssb_filepath(dates=dates,
                  frequency=frequency, 
                  name=name,
...
def save_ssb_file(df: pd.DataFrame, 
                  dates: tuple[int|None],
                  frequency: str, 
                  stable_version: bool,
                  name: str,
    ...
    """
    # Check content in df
    assert len(df) > 0, "There are no rows in the dataset. Fix this and try to save again."
...
def find_version_number(files:list[str], stable_version:bool) -> str:
    ...
    if not stable_version and len(files) == 0:
        ...
        print(f"FYI: A stable version {files[-1]} is saved previously. Saves as version 0.")
        return '0'
        ...
        overwrite = input(f"Do you want to overwrite the existing version (_v{existing_versions[-1]})? Confirm by 'y'")
        if overwrite.lower() == 'y':
            version = existing_versions[-1]
            version = (version.split('.')[0])
            return str(version)
        else:
            make_new_version =  input(f"Do you want to make a new version (i.e. increase the version number by one)? Confirm by 'y'")
        ...
        raise ValueError(f"Somethin went wrong when finding the version number.") 
...
def save_df(df: pd.DataFrame, 
    ...
    if version_number is None:
        ...
        # Uknown filetype sent as argument
            ...
            print(f"The filetype {filetype} is not supported for saving. Nothing saved for {file_path}.")
            
def structure_ssb_filepath(dates: tuple[int|None],
                  frequency: str, 
                  name: str,
                  datatilstand: str,
                  bucket_statistikk: str,  
                  folder_in_datatilstand: str = '',) -> str:
...
def get_files(folder_path: str,
    ...
    filenames = sorted(filenames, key=lambda x: int(re.search(r'_v(\d+)(?=\.(parquet|csv)$)', x).group(1)))
    return filenames
...
def verify_base_filename(name:str) -> str:
    ...
    if any(letter.isupper() for letter in name):
        old_name = name
        name = name.lower()
        print(f"Base filename changed from {old_name} to {name}. No upper case letters allowed.")
    # Raises error if starts with numbers
    if name[0].isdigit():
        raise ValueError(f"Base filename cannot start with a digit. Change it. \nCurrent base filename: {name}.")
    if 'parquet' in name:
        raise ValueError(f"The file format 'parquet' should not be specified in the base file name. Remove it.")
 
    if 'csv' in name:
        raise ValueError(f"The file format 'csv' should not be specified in the base file name. Remove it.")
    # Raises error if not valid signs are present in base file name
    not_valid_signs = ['.', ' ', '!', ',', 'æ', 'ø', 'å', '*', '^', '?', '#', '@', '%', '&', '(', ')']
    for letter in name:
        if letter in not_valid_signs:
            raise ValueError(f"Not valid signs present in base filename. Not valid signs are: {not_valid_signs}")
    return name
...
def verify_datatilstand(datatilstand: str) -> str:
    ...
    if datatilstand not in ['inndata', 'klargjorte-data', 'statistikk', 'utdata', 'temp']:
        datatilstand = input("Datatilstand må være enten inndata, klargjorte-data, statistikk eller utdata:")
        datatilstand = verify_datatilstand(datatilstand)
    else:
        return datatilstand